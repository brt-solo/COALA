...
TRAIN_PATH      = os.path.expanduser("~/MAP-CF/nhane/X_train_xgb_nhanes_diet.csv")
REFERENCE_PATH  = os.path.expanduser("~/MAP-CF/nhane/X_test_xgb_nhanes_diet.csv")
FEATURE_JSON    = os.path.join(os.path.dirname(os.path.abspath(__file__)), "feature_categories_multi_nhanes.json")
OUTPUT_DIR      = os.path.join(os.path.dirname(os.path.abspath(__file__)), "xgb_nhanes_output")
INIT_POP        = 1000
MAX_ITER        = 10000
METHOD          = "uniform"
MUTATION_RATE   = None
...
for idx, row in ref_df[X_train_df.columns].iterrows():
    print(f"Processing individual {idx} / {len(ref_df)}...")
...
out_path = os.path.join(OUTPUT_DIR, "counterfactuals_multi.pkl")
